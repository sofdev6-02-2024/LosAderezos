...
public class UserServiceTests
{
     private Mock<IUserDAO> _userDaoMock;
        ...
        {
            // Arrange
            var userWithoutId = new UserWithoutIdDTO { Name = "Test User" };
            var user = new User { UserId = Guid.NewGuid(), Name = "Test User" };
            var userDto = new UserDTO { UserId = user.UserId, Name = "Test User" };

            _mapperMock.Setup(m => m.Map<User>(It.IsAny<(UserWithoutIdDTO, Guid)>())).Returns(user);
            _userDaoMock.Setup(dao => dao.Create(user));
            _userDaoMock.Setup(dao => dao.Read(user.UserId)).Returns(user);
            _mapperMock.Setup(m => m.Map<UserDTO>(user)).Returns(userDto);
            ...
            Assert.IsNotNull(result);
            Assert.That(result.UserId, Is.EqualTo(userDto.UserId));
            _mapperMock.Verify(m => m.Map<User>(It.IsAny<(UserWithoutIdDTO, Guid)>()), Times.Once);
            _userDaoMock.Verify(dao => dao.Create(user), Times.Once);
            _userDaoMock.Verify(dao => dao.Read(user.UserId), Times.Once);
        }
